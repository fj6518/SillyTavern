...
if (!document.querySelector('#custom-toast-style')) {
  ...
  style.textContent = `
    .debug-scroll-toast .toast-message {
      max-height: 60vh;
      overflow-y: auto;
...
async function processTtsQueue() {
    ...
    if (extension_settings.tts.enable_custom_tags) {
        const processOrder = extension_settings.tts.tag_processing_order === 'whitelist_first'
            ? ['whitelist', 'blacklist']
            : ['blacklist', 'whitelist'];

        processOrder.forEach(step => {
            if (step === 'whitelist' && extension_settings.tts.whitelist_tags) {
                const whitelist = extension_settings.tts.whitelist_tags.split(',').map(t => t.trim().replace(/[<>]/g, ''));
                const tagNames = whitelist.join("|");
                const whitelistRegex = new RegExp(
                    `(<(${tagNames})(?:[^>]|(?!<${tagNames}))*>)((?:(?!<\/?${tagNames}).)*?)(<\/\\2>)`,
                    'gms'
                );

                text = (text.match(whitelistRegex) || []).join(' ');
                text = text.replace(whitelistRegex, '$1$3$4');
            }

            if (step === 'blacklist' && extension_settings.tts.blacklist_tags) {
                const blacklist = extension_settings.tts.blacklist_tags.split(',').map(t => t.trim().replace(/[<>]/g, ''));
                const blacklistRegex = new RegExp(`<(${blacklist.join('|')})[^>]*>.*?<\/\\1>`, 'gs');
                text = text.replace(blacklistRegex, '');
            }
        });
    }

    if (extension_settings.tts.skip_tags) {
        text = text.replace(/<([^\s>]+)(\s[^>]*)?>[\s\S]*?<\/\1>/gi, '').trim();
    }

    if (extension_settings.tts.remove_all_tags) {
        text = text.replace(/<[^>]+>/g, '');
    }
    ...
    if (extension_settings.tts.convert_quotes) {
        text = text.replace(/"/g, '＂');
    }
    ...
    if (extension_settings.tts.debug_mode) {
        const debugContent = `[DEBUG模式]\n角色卡: ${currentTtsJob.name}\n文本处理结果:\n${text}`;
        toastr.info(debugContent, '调试信息', {
          toastClass: 'debug-scroll-toast',
          timeOut: 0,
          extendedTimeOut: 0,
          closeButton: true
        });
...
function loadSettings() {
    ...
    $('#tts_convert_quotes').prop('checked', extension_settings.tts.convert_quotes);
    $('#tts_enable_custom_tags').prop('checked', extension_settings.tts.enable_custom_tags);
    $('#tts_whitelist_tags').val(extension_settings.tts.whitelist_tags);
    $('#tts_blacklist_tags').val(extension_settings.tts.blacklist_tags);
    $('#tts_remove_all_tags').prop('checked', extension_settings.tts.remove_all_tags);
    $('#tts_debug_mode').prop('checked', extension_settings.tts.debug_mode);
    $('#tts_tag_order').val(extension_settings.tts.tag_processing_order);
}
...
const defaultSettings = {
    ...
    debug_mode: false,
    tag_processing_order: 'whitelist_first',
};
...
jQuery(async function () {
    async function addExtensionControls() {
        ...
        $('#tts_enable_custom_tags').on('click', function() {
            extension_settings.tts.enable_custom_tags = $(this).prop('checked');
            saveSettingsDebounced();
        });

        $('#tts_whitelist_tags').on('change', function() {
            extension_settings.tts.whitelist_tags = $(this).val();
            saveSettingsDebounced();
        });

        $('#tts_blacklist_tags').on('change', function() {
            extension_settings.tts.blacklist_tags = $(this).val();
            saveSettingsDebounced();
        });

        $('#tts_remove_all_tags').on('click', function() {
            extension_settings.tts.remove_all_tags = $(this).prop('checked');
            saveSettingsDebounced();
        });

        $('#tts_convert_quotes').on('click', function() {
            extension_settings.tts.convert_quotes = $(this).prop('checked');
            saveSettingsDebounced();
        });

        $('#tts_debug_mode').on('click', function() {
            extension_settings.tts.debug_mode = $(this).prop('checked');
            saveSettingsDebounced();
        });

        $('#tts_tag_order').on('change', function() {
            extension_settings.tts.tag_processing_order = $(this).val();
            saveSettingsDebounced();
        });
