...
class SystemTtsProvider {
    ...
    async loadSettings(settings) {
        ...
        $('#system_tts_merge_generation').prop('checked', this.settings.merge_generation);

        $('#system_tts_merge_generation').on('click', () => {
            this.settings.merge_generation = $('#system_tts_merge_generation').prop('checked');
            saveTtsProviderSettings();
        });
    ...
    async generateTts(text, voiceId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.settings.merge_generation) {
                console.log('System TTS done (merged)');
                speechSynthesis.speak(utterance);
            } else {
                speechUtteranceChunker(utterance, {
                    chunkLength: 400,
                }, () => {
                    console.log('System TTS done (chunked)');
                });
            }
        });
    }
